...
use cards_proofs::{
    ...
    zkp::{arguments::shuffle, ArgumentOfKnowledge},
};

use ark_ff::Zero;
use ark_std::{iter::Iterator, rand::rngs::OsRng, UniformRand};
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let encrypt_parameters = Enc::setup(&mut rng);
    let (pk, _) = Enc::keygen(&encrypt_parameters, &mut rng);
    ...
    let prepare_proof_parameters = |m, n| {
        ...
        let permutation = Permutation::from_rng(&mut rng, number_of_ciphers);
    ...
    let num_of_chunks_x_chunk_length = vec![(4, 13), (13, 4), (2, 26), (26, 2), (6, 9)];
...

    {
        ...
        for (m, n) in num_of_chunks_x_chunk_length.clone() {
            ...
            let witness = Witness::new(&permutation, &masking_factors);
            let proof =
                ShuffleArgument::prove(&mut rng, &parameters, &statement, &witness, seed)
                    .unwrap();
            assert_eq!(
                Ok(()),
                ShuffleArgument::verify(&parameters, &statement, &proof, seed)
            );
            let bench_id =
                BenchmarkId::new("number_of_ciphers:", format!("({} * {} = {})", m, n, m * n));
            group.bench_function(bench_id, |b| {
                b.iter(|| ShuffleArgument::verify(&parameters, &statement, &proof, seed))
            });
        }
    }
}
