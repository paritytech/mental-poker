...
use ark_ff::{One};
use ark_std::{
    borrow::ToOwned, iter::Iterator,
    rand::{Rng,CryptoRng,thread_rng},
    UniformRand,
};
use std::collections::HashMap;
use cards_proofs::{
    // utils::{permutation::Permutation,rand::sample_vector},
    zkp::proofs::{chaum_pedersen_dl_equality},
};
use thiserror::Error;
...
use cards_protocol::{keys};
// type PublicKey = keys::PlayerPublicKey<Curve>;
// type SecretKey = keys::PlayerSecretKey<Curve>;
type Keypair = keys::PlayerKeypair<Curve>;

type Card = cards_protocol::UnmaskedCard<Curve>;
type MaskedCard = cards_protocol::MaskedCard<Curve>;
// type RevealToken = cards_protocol::RevealToken<Curve>;
type RevealMessage = cards_protocol::RevealMessage<Curve>;
...
impl Player {
    pub fn new<R: Rng+CryptoRng>(rng: &mut R, parameters: &CardParameters, name: &[u8]) -> anyhow::Result<Self> {
        let (pk,sk) = parameters.generate_player(rng,name);
        Ok(Self {
            name: name.to_owned(),
            pk,
            sk,
    ...
    pub fn peek_at_card(
        ...
        reveal_tokens: &mut Vec<RevealMessage>,
        ...
        let unmasked_card = parameters.unmask(reveal_tokens, card)?;
    ...
    pub fn compute_reveal_token<R: Rng+CryptoRng>(
        ...
        parameters.prove_single_reveal_token(rng, &self.sk, card)
    }
}
...
pub fn open_card(
    parameters: &CardParameters,
    reveal_tokens: &Vec<RevealMessage>,
    card_mappings: &HashMap<Card, ClassicPlayingCard>,
    card: &MaskedCard,
...
fn encode_cards<R: Rng+CryptoRng>(rng: &mut R, num_of_cards: usize) -> HashMap<Card, ClassicPlayingCard> {
...
fn main() -> anyhow::Result<()> {
    ...
    let parameters = CardParameters::setup(rng, num_of_cards);
    ...
    for p in players {
        apk.verify_n_add(p.pk,p.name.as_slice())?;
    }
    let joint_pk = apk.aggregate_key();
    let parameters = apk.parameters();  // Borrow this back to reduce changes.
    ...
    let deck_and_proofs: Vec<(MaskedCard, RemaskingProof)> = card_mapping
        .keys()
        .map(|card| parameters.prove_mask(rng, &joint_pk, &card, &Scalar::one()))
        .collect::<Vec<_>>();
